...
hook_foldable = function(type) {
  ...
  function(x, options) {
    ...
    return(res) 
      ...
      summary_title = options$label
      if (is.null(summary_title)) {
        summary_title = type
      }
      if (grepl(pattern = "unnamed", x = summary_title)) {
        summary_title = type
      } else {
...
General population hazard rates can be found in Life Tables. The life table of Jordan is shipped with the `NeuroblastomaPSM` package, and is used by default in the PSM model. The following steps describe the adjustment of the baseline mortality as it is employed in the `NeuroblastomaPSM` package.

### Generating the general population survival curve:

The survival curve represents the probability of surviving up to the corresponding age or time point, starting from a cohort with an initial size of 1 (or 100% survival). The `get_lifeTable_survival_curve` function calculates the survival curve from a given vector of mortality probabilities (the values recorded in any life table).
...
The 'cure threshold' in this PSM is employed via the `calculate_markov_trace` function. Therefore, part of the this function code is shown below to demonstrate how the 'cure threshold' assumptions are applied.
